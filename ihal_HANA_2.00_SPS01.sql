...
( SELECT
  ...
  ( SELECT 'ALL' ENV,     ' ' FILE_NAME,          ' ' SECTION,                ' ' PARAMETER_NAME,                          ' ' NOT_1, ' ' MIN_VALUE_1,      ' ' MAX_VALUE_1,         ' ' MIN_VALUE_2,           ' ' MAX_VALUE_2,           -1 MIN_REV, -1 MAX_REV, -1 MIN_VERS, -1 MAX_VERS, '' SAP_NOTE, ' ' AREA, ' 'TYPE FROM DUMMY WHERE 1 = 0 UNION ALL
    ...
    SELECT 'ALL',   'indexserver.ini',  'calcengine', 'optimize_simplify_filter_enabled',     ' ', '0',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  10.999, 2, 2, '2476216', 'indexserver_crash;calc_engine',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'lobhandling',  'garbage_collect_interval_s',     ' ', '0',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2477482', 'partitioning;LOB',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'resource_tracking',  'service_thread_sampling_monitor_include_passport',     ' ', 'no',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2478767', 'system_view',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'calcengine', 'instantiate_max_itab_transformation_size',     ' ', '<number_higher_than_number_of_predicates>',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2476064', 'calculation_view',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'session',  'fda_enable',     ' ', 'off',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2495247', 'FDA;wrong_result',  ' ' from DUMMY UNION ALL
    SELECT 'MULTI',   'indexserver.ini',  'distribution', 'client_distibution_mode',    ' ', 'off',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2500745', 'scale-out;LOB;repository;deploy',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'optimize_compression', 'singleindex_consider_for_compressed_columns',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2516807', 'indexes;CPU;join_engine',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'joins',  'disable_aggregate_simplification',     ' ', 'true',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2509750', 'partitioning',  ' ' from DUMMY UNION ALL
    SELECT 'SYSREP',  'global.ini',   'system_replication', 'enable_evict_pages_after_takeover',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 12,  12.009, 2, 2, '2519008', 'indexserver_crash;system_replication;page_cache',   ' ' from DUMMY UNION ALL
    SELECT 'SYSREP',  'indexserver.ini',  'row_engine', 'keep_shared_memory_over_restart',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2522580', 'indexserver_crash; system_replication;RowStore;delta_data_shipping',  ' ' from DUMMY UNION ALL
    SELECT 'BW',  'indexserver.ini',  'calcengine', 'ce2qo_enable_boundary_op',     ' ', '0',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2517443', 'performance; calc_view;Bex;memory_leak;FEMS;BWoH',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'search', 'late_materialization_threshold_for_insert',    ' ', '10000000000',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2539459', 'indexserver_crash;trex;late_materialization',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'metadata', 'ignore_exception_during_auto_migration',     ' ', 'on',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2535589', 'upgrade;indexserver_crash',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'query_mediator', 'filtertransport_enable',     ' ', '0',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2535996', 'indexserver_crash;olap_engine;optimizer',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'session',  'fda_enable',     ' ', 'off',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2534365', 'indexserver_crash;FDA',   ' ' from DUMMY UNION ALL
    SELECT 'ERP',   'indexserver.ini',  'sql',  'plan_cache_size',    ' ', '5368709120',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2523230', 'BSoH;error_2048;sql_plan_cache',  ' ' from DUMMY UNION ALL
    SELECT 'SYSREP',  'global.ini',   'system_replication', 'preload_column_tables',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2542853', 'indexserver_crash;system_replication;takeover;preload;delta_data_shipping;upgrade',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'session',  'fda_enable',     ' ', 'off',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2547605', 'indexserver_crash',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'session',  'protocol_trace',     ' ', '-- no recommendation --',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 11,  12.029, 2, 2, '2540710', 'indexserver_crash;PowerPC;trace',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'calcengine', 'disable_patterns',     ' ', '23',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.009, 2, 2, '2557609', 'calculation_view;calc_engine',  ' ' from DUMMY UNION ALL
    SELECT 'SDA',   'indexserver.ini',  'smart_data_access',  'enable_binary_transfer',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2566685', 'SDA;virtual_table',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'persistence',  'savepoint_max_pre_critical_flush_duration',    ' ', '-- no recommendation --',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2565285', 'performance;GC;LOB;MVCC',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'series', 'series_column_lrle_compression_enabled',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2549632', 'compression_LRLE',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'performance_analyzer', 'planviz_enable',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2542972', 'indexserver_crash;planviz',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'topology.ini',   '/databases', '2',    ' ', '-- no recommendation --',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2554185', 'MDC;SLD',   ' ' from DUMMY UNION ALL
    SELECT 'SDI',   'dpserver.ini',   'persistency',  'receiver_comi_interval_sec',     ' ', '2',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2560056', 'SDI',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'calcengine', 'ce2qo_disable_unfolding',    ' ', '1',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2544123', 'indexserver_crash;calc_engine;optimizer',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'backup', 'parallel_data_backup_backint_channels',    ' ', '-- no recommendation --',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2579371', 'indexserver_crash;nameserver_crash;backup,backint',   ' ' from DUMMY UNION ALL
    SELECT 'SYSREP',  'nameserver.ini',   'local_watchdog', 'interval',     ' ', '0',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2581373', 'system_replication;takeover',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'xsengine.ini',   'sql',  'use_new_placement_scheme_for_replicas',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2558683', 'XS_engine',   ' ' from DUMMY UNION ALL
    SELECT 'BW',  'indexserver.ini',  'calcengine', 'disable_patterns',     ' ', '38',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2100040', 'BWoH;CPU;performance',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'threads',  'default_stack_size_kb',    ' ', '2048',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2584287', 'indexserver_crash',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'threads',  'worker_stack_size_kb',     ' ', '2048',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2584287', 'indexserver_crash',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'expensive_statement',  'internal',     ' ', '-- no recommendation --',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2583885', 'indexserver_crash;trace',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'calcengine', 'disabled_patterns',    ' ', '43',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2584492', 'JDBC;Composite_Provider;calc_engine',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'nameserver.ini',   'session',  'max_number_of_sessions_per_partition',     ' ', '2048',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2516320', 'connection_limit;session',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'indexing', 'migrate_vf_to_ut_in_deltamerge',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2487855', 'consistency;delta_merge',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'persistence',  'savepoint_pre_critical_flush_retry_threshold',     ' ', '<clock tick value>',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.029, 2, 2, '2538561', 'performance',   ' ' from DUMMY UNION ALL
    SELECT 'BW',  'indexserver.ini',  'parallel', 'max_lock_retry',     ' ', '60',  '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2535656', 'OLAP:BWoH;SCM;lock_manager',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'performance_analyzer', 'planviz_enable',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2575166', 'indexserver_crash;planviz',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'communication',  'SSL',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.019, 2, 2, '2547142', 'OOM;SSL;encryption',  ' ' from DUMMY UNION ALL
    SELECT 'SDA',   'indexserver.ini',  'smart_data_access',  'join_relocation',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2587067', 'SDA;virtual_table',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'global.ini',   'resource_tracking',  'memory_tracking',    ' ', 'off',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  -1, 2, 2, '2586437', 'indexserver_crash;OOM;',  ' ' from DUMMY UNION ALL
    SELECT 'SDA',   'indexserver.ini',  'smart_data_access',  'join_relocation',    ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  12.039, 2, 2, '2585974', 'SDA;virtual_table;optimizer;Composite_provider',  ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'nameserver.ini',   'indexing', 'migrate_vf_to_ut_in_deltamerge',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2487855', 'consistency;delta_merge',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'indexserver.ini',  'optimize_compression', 'migrate_vf_to_ut',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2487855', 'consistency;delta_merge',   ' ' from DUMMY UNION ALL
    SELECT 'ALL',   'nameserver.ini',   'optimize_compression', 'migrate_vf_to_ut',     ' ', 'false',   '-- no recommendation --', '-- no recommendation --', '-- no recommendation --', 10,  11.999, 2, 2, '2487855', 'consistency;delta_merge',   ' ' from DUMMY UNION ALL
...
SELECT 'ALL','indexserver.ini','lobhandling','garbage_collect_interval_s',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2477482','partitioning;LOB',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','resource_tracking','service_thread_sampling_monitor_include_passport',' ','no','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2478767','system_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','instantiate_max_itab_transformation_size',' ','<number_higher_than_number_of_predicates>','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2476064','calc_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2495247','FDA;wrong_result',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','distribution','client_distibution_mode',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2500745','scale-out;LOB;repository;deploy',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','optimize_compression','singleindex_consider_for_compressed_columns',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2516807','indexes;CPU;join_engine',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','disable_aggregate_simplification',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2509750','partitioning',' ' from DUMMY UNION ALL
...
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2547605','indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','protocol_trace',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',11,12.029,2,2,'2540710','indexserver_crash;PowerPC;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','23','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.009,2,2,'2557609','calc_view;calc_engine',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','enable_binary_transfer',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2566685','SDA;virtual_table',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','persistence','savepoint_max_pre_critical_flush_duration',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2565285','performance;GC;LOB;MVCC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','series','series_column_lrle_compression_enabled',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2549632','compression_LRLE',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2542972','indexserver_crash;planviz',' ' from DUMMY UNION ALL
SELECT 'ALL','topology.ini','/databases','2',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2554185','MDC;SLD',' ' from DUMMY UNION ALL
SELECT 'SDI','dpserver.ini','persistence','receiver_comi_interval_sec',' ','2','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2560056','SDI',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','ce2qo_disable_unfolding',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2544123','indexserver_crash;calc_engine;optimizer',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','backup','parallel_data_backup_backint_channels',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2579371','indexserver_crash;nameserver_crash;backup,backint',' ' from DUMMY UNION ALL
SELECT 'SYSREP','nameserver.ini','local_watchdog','interval',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2581373','system_replication;takeover',' ' from DUMMY UNION ALL
SELECT 'ALL','xsengine.ini','sql','use_new_placement_scheme_for_replicas',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2558683','XS_engine',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','calcengine','disabled_patterns',' ','38','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2100040','BWoH;CPU;performance',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','threads','default_stack_size_kb',' ','2048','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2584287','indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','threads','worker_stack_size_kb',' ','2048','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2584287','indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','expensive_statement','internal',' ','-- no recommendation --','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2583885','indexserver_crash;trace',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','calcengine','disabled_patterns',' ','43','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2584492','JDBC;Composite_Provider;calc_engine;OLAP_engine;BWoH',' ' from DUMMY UNION ALL
SELECT 'ALL','nameserver.ini','session','max_number_of_sessions_per_partition',' ','2048','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2516320','connection_limit;session',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','indexing','migrate_vf_to_ut_in_deltamerge',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2487855','consistency;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','persistence','savepoint_pre_critical_flush_retry_threshold',' ','<clock tick value>','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2538561','performance',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','parallel','max_lock_retry',' ','60','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2535656','OLAP;BWoH;SCM;lock_manager',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2575166','indexserver_crash;planviz',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','communication','SSL',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2547142','OOM;SSL;encryption',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2587067','SDA;virtual_table',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','resource_tracking','memory_tracking',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2586437','indexserver_crash;OOM;',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','join_relocation',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.039,2,2,'2585974','SDA;virtual_table;optimizer;Composite_provider',' ' from DUMMY UNION ALL
SELECT 'ALL','nameserver.ini','indexing','migrate_vf_to_ut_in_deltamerge',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2487855','consistency;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','optimize_compression','migrate_vf_to_ut',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2487855','consistency;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','nameserver.ini','optimize_compression','migrate_vf_to_ut',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2487855','consistency;delta_merge',' ' from DUMMY UNION ALL
...
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_execution_count',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_execution_count_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_execution_time',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_execution_time_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_memuse',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_memuse_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_part_id',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_part_id_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_partnum',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_partnum_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_rows',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_rows_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_classification',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_classification_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_size_hosted',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_size_hosted_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_subclassification',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'MULTI','indexserver.ini','table_placement','balance_by_table_subclassification_weight',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','parameter;redistribution;MDC;scale-out',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','calcengine','disabled_patterns',' ','23','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.049,2,2,'2601405','calc_engine;calc_view',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','communication','default_read_timeout',' ','-1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2488737','network;TREX;tcp_timeout',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','planningengine','formula_parallel_limit',' ','<large_integer_value>','-- no recommendation --','-- no recommendation --','-- no recommendation --',12.02,12.049,2,2,'2607741','wrong_result;planning_engine',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','dso','search_realization',' ','DSOSearchUsingSearchAPI','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.049,2,2,'2613469','BWoH;DSO;indexserver_crash',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','search','collect_top_documents_count',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2611281','attribute_engine;',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','joins','single_thread_execution_for_partitioned_tables',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2620310','join_engine;performance;delta_merge',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','sqlscript','typecheck_procedure_input_param',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2457876','calc_engine;calc_view',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','memorymanager','statement_memory_limit',' ','<increase_by_1GB>','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2603589','oom;memory_leak',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','resource_tracking','enable_tracking',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2603589','oom;memory_leak',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','resource_tracking','memory_tracking',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.029,2,2,'2603589','oom;memory_leak',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','remote_conn_idle_timeout’',' ','600','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2578523','SDA;tcp_timeout;connection_limit',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','session','socket_keepalive',' ','on','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2621464','tcp_timeout;BWoH;error_10107;error_10108;network',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','session','tcp_keepalive_time',' ','60','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2621464','tcp_timeout;BWoH;error_10107;error_10108;network',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','session',' tcp_keepalive_intvl',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2621464','tcp_timeout;BWoH;error_10107;error_10108;network',' ' from DUMMY UNION ALL
SELECT 'BW','indexserver.ini','session','tcp_keepalive_probes',' ','5','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2621464','tcp_timeout;BWoH;error_10107;error_10108;network',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','communication','ssl',' ','systempki','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2618451','SSL;encryption;MDC;system_replication',' ' from DUMMY UNION ALL
SELECT 'SYSREP','global.ini','system_replication_communication','enable_ssl',' ','on','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2618451','SSL;encryption;MDC;system_replication',' ' from DUMMY UNION ALL
SELECT 'SYSREP','indexserver.ini','distribution','systemreplication_protocol_version',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.049,2,2,'2587258','hana_client;system_replication',' ' from DUMMY UNION ALL
SELECT 'SDI','indexserver.ini','session','max_statements_per_connection',' ','200000','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,11.999,2,2,'2464140','SDI;data_provisioning;connection_limit',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','session','fda_enable',' ','off','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2627679','FDA;workprocess;signal_11',' ' from DUMMY UNION ALL
SELECT 'SDA','indexserver.ini','smart_data_access','enable_or_to_in',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',12.01,12.049,2,2,'2601496','SDA;virtual_table;ODBC',' ' from DUMMY UNION ALL
SELECT 'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2637828','memory_leak;performance;planviz;trace',' ' from DUMMY UNION ALL
SELECT 'ALL','global.ini','performance_analyzer','plan_trace_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2643064','indexserver_crash;trace;planviz',' ' from DUMMY UNION ALL
