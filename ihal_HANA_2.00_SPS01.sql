...
SELECT 4,'SYSREP','nameserver.ini','sld','enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2577511','SLD;system_replication',' ' from DUMMY UNION ALL
SELECT 4,'ALL','nameserver.ini','sld','enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,12.019,2,2,'2577511','SLD;MDC',' ' from DUMMY UNION ALL
SELECT 1,'ALL','indexserver.ini','performance_analyzer','planviz_enable',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','indexserver_crash;planviz',' ' from DUMMY UNION ALL
SELECT 2,'ALL','indexserver.ini','transaction','lock_wait_timeout',' ','5*<timeout_in_ms>','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2549013','lock_manager;deadlock;XS_engine',' ' from DUMMY UNION ALL
SELECT 1,'ALL','indexserver.ini','sql','cs_equi_cond_update_optimization_batch_size',' ','1000000','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2644935','LOB;wrong_result;consistency',' ' from DUMMY UNION ALL
SELECT 3,'ALL','nameserver.ini','statisticsserver','active',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2644242','statisticsserver;system_view;upgrade',' ' from DUMMY UNION ALL
SELECT 2,'ALL','indexserver.ini','statisticsserver','threadpool',' ','0','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2584388','Memory_leak;',' ' from DUMMY UNION ALL
SELECT 3,'ALL','global.ini','backup','data_backup_buffer_size',' ','1536','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2657261','backup;backint;performance',' ' from DUMMY UNION ALL
...
SELECT 3,'SYSREP','global.ini','system_replication','check_secondary_active_status',' ','false','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2649721','system_replication;lock_manager;system_view',' ' from DUMMY UNION ALL
SELECT 4,'ALL','global.ini','resource_tracking','service_thread_sampling_monitor_enabled',' ','true','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2600030','thread;monitoring',' ' from DUMMY UNION ALL
SELECT 2,'ALL','indexserver.ini','calcengine','enforce_bigint_counters',' ','1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2648134','calc_view;calc_engine;attribute_engine',' ' from DUMMY UNION ALL
SELECT 1,'MULTI','indexserver.ini','search','late_materialization_threshold',' ','-1','-- no recommendation --','-- no recommendation --','-- no recommendation --',10,-1,2,2,'2662717','indexserver_crash;scale-out;late_materialization',' ' from DUMMY UNION ALL
